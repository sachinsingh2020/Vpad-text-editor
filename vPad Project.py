...
main_application.title('Sachin text editor')
...
def change_bold():
    global bold_status
    print("enter in change bold function")
    text_property = tk.font.Font(font=text_editor['font'])
    if text_property.actual()['weight'] == 'normal':
        print("enter in text_property funtion")
        bold_status = 'bold'
        print(bold_status)
        text_editor.configure(
            font=(current_font_family, current_font_size, bold_status))
...
def change_italic():
    global italic_status
    text_property = tk.font.Font(font=text_editor['font'])
    print(tk.font.Font(font=text_editor['font']).actual())
    if text_property.actual()['slant'] == 'roman':
        italic_status = 'italic'
        text_editor.configure(
            font=(current_font_family, current_font_size, italic_status))
...
# underline button
underline_status = 'normal'


def change_underline():
    global underline_status
    text_property = tk.font.Font(font=text_editor['font'])
    if text_property.actual()['underline'] == 0:
        underline_status = 'underline'
        text_editor.configure(
            font=(current_font_family, current_font_size, underline_status))
...
def open_file(event=None):
    ...
    try:
        print("in try")
        with open(url, 'r', encoding="utf-8-sig") as fr:
            print("in try open")
            text_editor.delete(1.0, tk.END)
    ...
    def find():
        ...
        if word:
            ...
            while True:
                ...
                print(f'start_pos: {start_pos}', end=' ')
                if not start_pos:
                    break
                end_pos = f'{start_pos}+{len(word)}c'
                print(f'end_pos: {end_pos}', end=' ')
                print()
...
view.add_checkbutton(label='Tool Bar', onvalue=True, offvalue=False,
                     variable=show_toolbar, image=tool_bar_icon, compound=tk.LEFT, command=hide_toolbar)
...
# color theme radiobuttons and functionality
